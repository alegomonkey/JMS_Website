# JMS_Website — Setup & Deployment

Detailed setup, deployment, admin, and security guide for the JMS_Website
project. For a project overview, tools at a glance, and the 60-second local
quickstart, see the top-level [README](../README.md).

## Capabilities

- **Vertical nav** (desktop) with Home / Projects / Settings (gear) / Sign-in.
  Below 768 px the nav collapses to a hamburger drawer with focus trap,
  Esc-to-close, and focus restoration.
- **Landing page** (`/`): hero image, bio, link tree, contact cards, and an
  embedded resume PDF (with a download fallback). No separate CV page —
  the resume lives on Home.
- **Projects index** (`/projects`): tag-filter combobox with full keyboard
  navigation (`aria-activedescendant` pattern), active-filter chips, paginated
  6 projects per page, all driven by URL search params so links are shareable.
- **Project detail** (`/projects/:slug`): per-project overview, contributions,
  documentation, repo link, plus a threaded discussion (comments are
  upvoteable; default sort is highest voted; toggle to newest).
- **Auth**: sign-in / register / sign-out. Guests can read everything;
  posting and voting require an account.
- **Admin role**: a single privileged role can delete any comment. Promotion
  is a CLI script run on the host — see [Admin access](#admin-access).
- **Theming**: dark mode is the default but the initial theme follows the OS
  `prefers-color-scheme` when no stored preference exists. Settings has a
  switch to override. CSS variables in `web/src/theme/tokens.css` define the
  palette and breakpoint tokens.
- **Accessibility**: WCAG 2.2 AAA target. Skip-to-content link, AAA focus
  ring, 44×44 px touch targets, `prefers-reduced-motion` respected,
  axe-tested per route. See [`web/ACCESSIBILITY.md`](../web/ACCESSIBILITY.md)
...
- **Testing**: Vitest + Supertest (API), Vitest + React Testing Library +
  `jest-axe` (Web)
...
JMS_Website/
  ...
  web/                  Vite + React frontend
    ACCESSIBILITY.md    Manual a11y checklist
  nginx/                Dockerfile, config template, entrypoint
  docs/                 Long-form notes
  setup/                This deployment guide
  docker-compose.yml
  .env.example
...
cd web && npm test     # Web: auth, vote button, NavBar drawer, combobox
                       #      keyboard, plus axe a11y smoke per page
```

The web tests include automated WCAG checks via `jest-axe`. Each page
(`Landing`, `Projects`, `ProjectDetail`, `SignIn`, `Register`, `Settings`)
has an `*.a11y.test.tsx` file that asserts `axe(container)` returns no
violations.
